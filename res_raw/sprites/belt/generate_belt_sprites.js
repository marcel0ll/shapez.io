...
async function run() {
    ...
    for (let c = 0; c < colors.length; c++) {
        ...
        const arrowColor = color.value;
        const arrowColorKey = color.key ? `${color.key}_` : "";
        ...
        for (let i = 0; i < fps; ++i) {
            ...
            const out = fs.createWriteStream(path.join(__dirname, "forward_" + arrowColorKey + i + ".png"));
        ...
        for (let i = 0; i < fps; ++i) {
            ...
            const out = fs.createWriteStream(path.join(__dirname, "right_" + arrowColorKey + i + ".png"));
            const stream = canvas.createPNGStream();
            stream.pipe(out);

            const outLeft = fs.createWriteStream(path.join(__dirname, "left_" + arrowColorKey + i + ".png"));
